...
#   RMSE: 6.7733  MAE: 4.8238
# =======================================================
#   Day1  RMSE=6.1115  MAE=4.2987
#   Day2  RMSE=7.0015  MAE=5.0024
#   Day3  RMSE=7.2068  MAE=5.1704
...
def load_and_split_data():
    ...
    print(f"訓練集: {n_train} | 驗證集: {n_val} | 測試集: {len(Xobs_te_n)} | ★最近格點CAMS + 座標Fourier + 時間embedding + log1p+z-score + 原始觀測目標")
    return {
...
if __name__ == "__main__":
    ...
    aux_val = coords_ff.unsqueeze(0).expand(S_val, -1, -1).reshape(-1, 64)   # ★fourier版：座標Fourier(64維)，與 aux_tr/aux_te 一致
    ...
    MAX_EPOCHS = 800
    BEST_PATH = "best_cnn_cams_timeembed_fourier_nearest_earlystop.pth"; CKPT_PATH = "cnn_cams_timeembed_fourier_nearest_earlystop_checkpoint.pth"
    ...
    save_predictions_csv("pred_cnn_timeembed_fourier_nearest_earlystop.csv", P_3d, T_3d, C_3d,
                         data['t0_test'], data['station_names'])

    try:
        from compare_models import dump_wide_csv
        if len(data['t0_test']) == S_te:
            dump_wide_csv("pred_cnn_timeembed_fourier_nearest_earlystop_wide.csv", P_3d, T_3d, C_3d, data['t0_test'], data['station_names'])
