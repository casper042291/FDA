...
#   [模型]     RMSE: 6.5704  MAE: 4.6839
#   [CAMS原始] RMSE: 19.5213  MAE: 14.1195
# ============================================================
#   第1天 (+01~+24h)  模型 RMSE: 6.0038 MAE: 4.2544  |  CAMS RMSE: 18.4506
#   第2天 (+25~+48h)  模型 RMSE: 6.7565 MAE: 4.8283  |  CAMS RMSE: 19.7715
#   第3天 (+49~+72h)  模型 RMSE: 6.9510 MAE: 4.9690  |  CAMS RMSE: 20.3418

# [參考] 先站後平均(有效站 72/72)  RMSE: 6.5233  MAE: 4.6839  R²: 0.4528
import pandas as pd
import numpy as np
import torch
...
def build_x96(Xpast_n_pm, Xpast_n_wx, A_norm):
    """★歷史氣象:過去24h = pm2.5+氣象;未來72h = CAMS + 氣象持續性(過去24h氣象×3)。
       未來段僅用『歷史氣象之持續性外推』,不含任何未來氣象觀測(無洩漏)。"""
    n, N = A_norm.shape[0], A_norm.shape[1]
    fut_wx = Xpast_n_wx.repeat(1, 1, 3, 1)                       # [n,N,72,n_wx] 過去24h氣象×3
...
def load_and_split_data(fpca_file, raw_file, station_file, weather_files, cams_npz):
    ...
    X96_all   = build_x96(past_pm, past_wx, A_tr_norm)
    ...
    if Xp_te is not None and len(Xp_te) > 0:
        ...
        X96_te = build_x96(past_pm_te, past_wx_te, A_te_norm)
...
if __name__ == "__main__":
    ...
    best_path = "best_ffno_3d_72h_v9_unseen_2_weather24_auxmlp_10_44_earlystop.pth"  # ★早停版專用(勿覆蓋原檔)
    print(f"\n=== 開始訓練(座標版,全72站,★含歷史氣象,+★early stopping,patience={PATIENCE},上限{MAX_EPOCH})===")
    stopped = False
