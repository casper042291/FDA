...
# ============================================================
#   三天整體 — 中央論文 per-hour pooled（座標版,全 72 站,CAMS最近格點）
#   [模型]     RMSE: 6.5003  MAE: 4.6912
#   [CAMS原始] RMSE: 19.5213  MAE: 14.1195
# ============================================================
#   第1天 (+01~+24h)  模型 RMSE: 5.8803 MAE: 4.1779  |  CAMS RMSE: 18.4506
#   第2天 (+25~+48h)  模型 RMSE: 6.6852 MAE: 4.8579  |  CAMS RMSE: 19.7715
...
def _open_ascii_nc(path):
    """netCDF4 在含中文/特殊字元路徑時可能開不了（Windows 常見），先複製到 ASCII 暫存
    路徑再開，比照 build_cams_pm25.py 的 open_ascii()。"""
...
def build_cams_nearest_npz(raw_nc_dir, station_csv, out_npz, ref_npz):
    ...
    逐 起報(12UTC) x horizon(+1~+72h) x 測站。
    邏輯對照 build_cams_pm25.py 的 build_perinit()，只有 .interp() 的 method
    從預設 'linear' 改為 'nearest'；僅在快取不存在時執行一次。
    ★測站清單與站序以 ref_npz(bilinear 錨點) 為準，座標查自 station_csv，確保 nearest 與
      bilinear 兩檔「只差在內插方法」；station_csv 若缺任一站座標 → 報錯中止。"""
    import glob
    import xarray as xr

    # ★以 bilinear 錨點 npz 的站名與站序為準（維持消融唯一變因）
    ...
    if not files:
        sys.exit(f"錯誤：找不到原始 CAMS 網格 netCDF：{raw_nc_dir}/*.nc"
                  ...
                  f"請確認 cams_raw_nc_dir 是否指向正確目錄）")
    ...
    for i, f in enumerate(files):
        ...
        arr = interp.values                                               # (D,72,station)
        vt  = ds['valid_time'].transpose('forecast_reference_time', 'forecast_period').values
        frt = ds['forecast_reference_time'].values                        # (D,) 12UTC 起報
        ds.close()
        order = np.argsort(frt)                                           # 確保起報遞增
        arr, vt = arr[order], vt[order]

        for d in range(arr.shape[0]):
            valid_local = pd.to_datetime(vt[d]) + pd.Timedelta(hours=8)   # (72,) UTC -> 本地
            pm_blocks.append(arr[d])                                      # (72,station)
            valid_list.append(valid_local.values)
        if (i + 1) % 12 == 0 or i == len(files) - 1:
            print(f"[CAMS-nearest]  已處理 {i+1}/{len(files)}  累積起報 {len(pm_blocks):,}")

    pm25 = np.stack(pm_blocks, axis=0).astype(np.float32)                 # (n_init,72,station)
    valid_local = np.stack(valid_list, axis=0)                            # (n_init,72)
...
if __name__ == "__main__":
    ...
    data = load_and_split_data(fpca_pm25_file, raw_pm25_file, station_file, weather_files,
                               cams_npz_nearest_file)   # ★改用最近格點版 CAMS 錨點
    ...
    best_path = "best_ffno_3d_72h_v9_unseen_2_auxmlp_10_44_camsnearest_earlystop.pth"  # ★早停版專用(勿覆蓋原檔)

    # ★[穩定性] 週期性續訓 checkpoint：萬一 segfault/核心傾印，重跑本檔即可從上次續訓
    #   （續訓點一併保存 early-stopping 狀態：best_val/best_ep/epochs_no_improve）。
    #   ※全新開跑(無 resume 檔)時 start_ep=1；想從頭訓練把 resume_path 刪掉即可。
    resume_path = "resume_v9_unseen_2_camsnearest_earlystop.pt"
    SAVE_EVERY = 20                                        # 每 N epoch 存一次續訓點
    ...
    if os.path.exists(resume_path):
        ...
        best_val = ckpt['best_val']; best_ep = ckpt['best_ep']
        epochs_no_improve = ckpt['epochs_no_improve']
        start_ep = ckpt['epoch'] + 1
        print(f"[resume] 從續訓點載入：已完成 epoch {ckpt['epoch']} → 從 {start_ep} 繼續 "
              f"(best_val={best_val:.6f}@{best_ep}, noImp={epochs_no_improve})")

    print(f"\n=== 開始訓練(座標版,全72站,無氣象,CAMS最近格點,+★early stopping,patience={PATIENCE},上限{MAX_EPOCH})===")
    stopped = False
    for ep in range(start_ep, MAX_EPOCH + 1):
        # --- 訓練 ---
        ...
        # --- [穩定性] 週期性存續訓點（含 early-stopping 狀態，供 segfault 後續訓）---
        if ep % SAVE_EVERY == 0:
    ...
    if test_loader:
        ...
        print("=" * 60)
        print("  三天整體 — 中央論文 per-hour pooled（座標版,全 72 站,CAMS最近格點）")
